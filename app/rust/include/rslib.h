...
parser_error_t rs_parse_canister_call_request(const uint8_t *data, uint16_t data_len);
...
parser_error_t rs_verify_certificate(const uint8_t *certificate, uint16_t certificate_len, const uint8_t *root_key);

parser_error_t rs_getNumItems(uint8_t *num_items);

parser_error_t rs_getItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outValue, uint16_t outValueLen,
