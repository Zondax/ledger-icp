...
uint8_t const DER_PREFIX[] = {0x30, 0x56, 0x30, 0x10, 0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02,
    ...
    switch (txtype) {
        ...
        case state_transaction_read: {
            ...
            for (size_t index = 0; index < fields->paths.arrayLen; index++) {
                cx_hash_sha256((uint8_t *)fields->paths.paths[index].data, fields->paths.paths[index].len,
                               arrayBuffer + index * CX_SHA256_SIZE, CX_SHA256_SIZE);
            }
...
zxerr_t crypto_sign_bls(uint8_t *signatureBuffer, uint16_t signatureMaxlen, uint16_t *sigSize, uint8_t *payload,
                        uint16_t payloadLen) {
    ...
    zxerr_t err = zxerr_ledger_api_error;
    // Generate keys
    CATCH_CXERROR(os_derive_bip32_no_throw(CX_CURVE_SECP256K1, hdPath, HDPATH_LEN_DEFAULT, privateKeyData, NULL));
    CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(CX_CURVE_SECP256K1, privateKeyData, 32, &cx_privateKey));

    // Sign
    CATCH_CXERROR(cx_ecdsa_sign_no_throw(&cx_privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, message_digest, CX_SHA256_SIZE,
...
zxerr_t crypto_sign(uint8_t *signatureBuffer, uint16_t signatureMaxlen, uint16_t *sigSize) {
    if (signatureMaxlen < SIGN_PREHASH_SIZE + sizeof(signature_t)) {
        return zxerr_buffer_too_small;
    }
...
// END:
// DIGEST_REQUEST || SIGNATURE_REQUEST || DIGEST_STATEREAD ||
// SIGNATURE_STATEREAD

zxerr_t crypto_sign_combined(uint8_t *signatureBuffer, uint16_t signatureMaxlen, uint8_t *predigest_request,
                             uint8_t *predigest_stateread, uint16_t *sigSize) {
    ...
    uint8_t privateKeyData[64] = {0};
    uint32_t info = 0;
    uint32_t sigLen = sizeof_field(signature_t, der_signature);
    ...
    // Sign request
    CATCH_CXERROR(cx_ecdsa_sign_no_throw(&cx_privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256, signatureBuffer,
                                         CX_SHA256_SIZE, sigma.der_signature, &sigLen, &info));
    ...
    if (err_c != no_error) {
        ...
        sigLen = sizeof_field(signature_t, der_signature);
        info = 0;

        // Sign stateread
        CATCH_CXERROR(cx_ecdsa_sign_no_throw(&cx_privateKey, CX_RND_RFC6979 | CX_LAST, CX_SHA256,
                                             signatureBuffer + CX_SHA256_SIZE + SIGNATURE_SIZE_RS, CX_SHA256_SIZE,
                                             sigma.der_signature, &sigLen, &info));
...
zxerr_t crypto_getDigest(uint8_t *digest, txtype_e txtype) { return zxerr_ok; }
...
zxerr_t crypto_extractPublicKey(uint8_t *pubKey, uint16_t pubKeyLen) {
    const uint8_t publicKey[SECP256K1_PK_LEN] = {
        0x04, 0x10, 0xD3, 0x49, 0x80, 0xA5, 0x1A, 0xF8, 0x9D, 0x33, 0x31, 0xAD, 0x5F, 0xA8, 0x0F, 0xE3, 0x0D,
        0x88, 0x68, 0xAD, 0x87, 0x52, 0x64, 0x60, 0xB3, 0xB3, 0xE1, 0x55, 0x96, 0xEE, 0x58, 0xE8, 0x12, 0x42,
        0x29, 0x87, 0xD8, 0x58, 0x9B, 0xA6, 0x10, 0x98, 0x26, 0x4D, 0xF5, 0xBB, 0x9C, 0x2D, 0x3F, 0xF6, 0xFE,
        0x06, 0x17, 0x46, 0xB4, 0xB3, 0x1A, 0x44, 0xEC, 0x26, 0x63, 0x66, 0x32, 0xB8, 0x35};
...
/*
 * neuron_sub_account =
     sha256(0x0c | “neuron-stake” | neuron_holder_principal |
neuron_creation_memo) to.hash = account_identifier(
...
zxerr_t crypto_computeStakeSubaccount(const uint8_t *principal, uint16_t principalLen, const uint8_t *memo, uint16_t memoLen,
                                      uint8_t *subaccount, uint16_t subaccountLen) {
    if (principalLen > DFINITY_PRINCIPAL_LEN || subaccountLen < DFINITY_SUBACCOUNT_LEN || memoLen > sizeof(uint64_t)) {
        return zxerr_invalid_crypto_settings;
    }
...
zxerr_t crypto_principalToStakeAccount(const uint8_t *principal, uint16_t principalLen, const uint64_t neuron_creation_memo,
    ...
    CHECK_ZXERR(hash_sha224((uint8_t *)final_hash, sizeof(stake_account_hash), address + 4, (maxoutLen - 4)));
...
// CRC-32(b) || b with b = SHA-224(“\x0Aaccount-id“ || owner || sub-account),
// where owner is a (29-byte)
zxerr_t crypto_principalToSubaccount(const uint8_t *principal, uint16_t principalLen, const uint8_t *subAccount,
                                     uint16_t subaccountLen, uint8_t *address, uint16_t maxoutLen) {
    ...
    CHECK_ZXERR(hash_sha224(hashinput, SUBACCOUNT_PREFIX_SIZE + principalLen + DFINITY_SUBACCOUNT_LEN, address + 4,
                            (maxoutLen - 4)));
...
zxerr_t crypto_fillAddress(uint8_t *buffer, uint16_t buffer_len, uint16_t *addrLen) {
    ...
    CHECK_ZXERR(crypto_principalToSubaccount(answer->principalBytes, sizeof_field(answer_t, principalBytes), zero_subaccount,
                                             DFINITY_SUBACCOUNT_LEN, answer->subAccountBytes,
