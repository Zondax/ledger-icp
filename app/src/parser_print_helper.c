...
parser_error_t format_principal_with_delimiters(const char *input,
...
parser_error_t
page_textual_with_delimiters(const char *input, const uint16_t inputLen,
  ...
  const size_t inputStrLen =
      strnlen(input, inputLen); // not including null terminator
  *pageCount = (uint8_t)(inputStrLen / CHARS_PER_PAGE) +
  ...
  for (uint8_t idx = 0; idx < CHUNKS_PER_PAGE; idx++) {
    ...
    const size_t remainingChars =
        inputStrLen - (pageIdx * CHARS_PER_PAGE) - (idx * CHARS_PER_CHUNK);
    const bool endOfInput =
        remainingChars <= 5; // strnlen does not count null terminator
    const bool skipDash = (idx % 3 == 2);
...
parser_error_t print_u64(uint64_t value, char *outVal, uint16_t outValLen,
                         uint8_t pageIdx, uint8_t *pageCount) {
  char buffer[100];
...
parser_error_t print_ICP(uint64_t value, char *outVal, uint16_t outValLen,
                         uint8_t pageIdx, uint8_t *pageCount) {
  char buffer[200] = {0};
...
parser_error_t print_Amount(uint64_t value, char *outVal, uint16_t outValLen,
                            uint8_t pageIdx, uint8_t *pageCount,
                            uint8_t decimals) {
  char buffer[200] = {0};
...
parser_error_t print_principal(const uint8_t *data, uint16_t len, char *outVal,
                               uint16_t outValLen, uint8_t pageIdx,
                               uint8_t *pageCount) {
  char tmpBuffer[100] = {0};
  uint16_t outLen = sizeof(tmpBuffer);
  zxerr_t err =
      crypto_principalToTextual(data, len, (char *)tmpBuffer, &outLen);
  if (err != zxerr_ok) {
    return parser_unexpected_error;
  }

  return page_textual_with_delimiters(tmpBuffer, outLen, outVal, outValLen,
                                      pageIdx, pageCount);
}

parser_error_t parser_printDelay(uint64_t value, char *buffer,
                                 uint16_t bufferSize) {
  ...
  if (days > 0) {
    if (index > 0) {
      PARSER_ASSERT_OR_ERROR(index + 2 < bufferSize,
...
parser_error_t format_principal(const uint8_t *data, uint16_t len, char *outVal,
                                uint16_t outValLen) {
  if (outValLen < 35) {
    return parser_unexpected_buffer_end;
  }
