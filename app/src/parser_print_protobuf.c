...
__Z_INLINE parser_error_t print_accountBytes(sender_t sender, const SendRequest *sendrequest, char *outVal,
                                             uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    uint8_t address[32];
    MEMZERO(address, sizeof(address));

    zxerr_t err = crypto_principalToSubaccount(sender.data, (uint16_t)sender.len, sendrequest->from_subaccount.sub_account,
                                               32, address, sizeof(address));
    if (err != zxerr_ok) {
...
static parser_error_t parser_getItemTokenTransfer(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                  uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    ...
    *pageCount = 1;
    ...
    if (app_mode_expert()) {
        ...
        if (displayIdx == 2) {
            ...
            if (fields->data.SendRequest.has_from_subaccount) {
                return page_hexstring_with_delimiters(fields->data.SendRequest.from_subaccount.sub_account, DFINITY_ADDR_LEN,
    ...
    if (displayIdx == 1) {
        ...
        return print_accountBytes(fields->sender, &fields->data.SendRequest, outVal, outValLen, pageIdx, pageCount);
    }

    if (displayIdx == 2) {
        PARSER_ASSERT_OR_ERROR(fields->data.SendRequest.has_to, parser_unexpected_number_items)
        snprintf(outKey, outKeyLen, "To account ");
        return page_hexstring_with_delimiters(fields->data.SendRequest.to.hash, 32, outVal, outValLen, pageIdx, pageCount);
    }
    ...
    if (displayIdx == 3) {
        ...
        return print_ICP(fields->data.SendRequest.payment.receiver_gets.e8s, outVal, outValLen, pageIdx, pageCount);
    }
    ...
    if (displayIdx == 4) {
        ...
        return print_ICP(fields->data.SendRequest.max_fee.e8s, outVal, outValLen, pageIdx, pageCount);
    }
...
static parser_error_t parser_getItemStakeNeuron(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    MEMZERO(outKey, outKeyLen);
    MEMZERO(outVal, outValLen);
    snprintf(outKey, outKeyLen, "?");
    ...
    *pageCount = 1;
    ...
    if (app_mode_expert()) {
        ...
        if (displayIdx == 2) {
            snprintf(outKey, outKeyLen, "Subaccount ");
            if (fields->data.SendRequest.has_from_subaccount) {
                return page_hexstring_with_delimiters(fields->data.SendRequest.from_subaccount.sub_account, 32, outVal,
...
static parser_error_t parser_getItemListNeurons(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                uint16_t outValLen) {
    if (displayIdx == 0) {
        snprintf(outKey, outKeyLen, "Transaction type");
...
static parser_error_t parser_getItemIncreaseNeuronTimer(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                        uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemJoinNeuronsFund(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                    uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemStartStopDissolve(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                      uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemSpawn(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                          uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemAddRemoveHotkey(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                    uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemDisburse(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                             uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
    ...
    if (displayIdx == 2) {
        ...
        return page_hexstring_with_delimiters(fields->command.disburse.to_account.hash.bytes,
                                              fields->command.disburse.to_account.hash.size, outVal, outValLen, pageIdx,
                                              pageCount);
    }
...
static parser_error_t parser_getItemRegisterVote(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                 uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemFollow(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                           uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    *pageCount = 1;
...
static parser_error_t parser_getItemManageNeuron(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                                 uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
    MEMZERO(outKey, outKeyLen);
    MEMZERO(outVal, outValLen);
    snprintf(outKey, outKeyLen, "?");
    ...
    *pageCount = 1;
    ...
    switch (mn_type) {
        case Configure_IncreaseDissolveDelay:
            return parser_getItemIncreaseNeuronTimer(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
        case Configure_JoinNeuronsFund:
            return parser_getItemJoinNeuronsFund(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
        ...
        case Spawn: {
            return parser_getItemSpawn(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
        }
...
parser_error_t parser_getItemProtobuf(uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal, uint16_t outValLen,
    ...
    *pageCount = 1;
    switch (parser_tx_obj.tx_fields.call.method_type) {
        case pb_sendrequest: {
            ...
            if (is_stake_tx) {
                return parser_getItemStakeNeuron(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
            }

            return parser_getItemTokenTransfer(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
        }

        case pb_manageneuron: {
            return parser_getItemManageNeuron(displayIdx, outKey, outKeyLen, outVal, outValLen, pageIdx, pageCount);
        }

        case pb_listneurons: {
            return parser_getItemListNeurons(displayIdx, outKey, outKeyLen, outVal, outValLen);
        }
