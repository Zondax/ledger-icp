...
import { ResponseAddress, ResponseAppInfo, ResponseDeviceInfo, ResponseSign, ResponseSignUpdateCall, ResponseVersion } from './types'
import {
  ADDRLEN,
  CHUNK_SIZE,
  CLA,
...
export default class InternetComputerApp {
  ...
  async showAddressAndPubKey(path: string): Promise<ResponseAddress> {
    ...
    return this.transport
      .send(CLA, INS.GET_ADDR_SECP256K1, P1_VALUES.SHOW_ADDRESS_IN_DEVICE, 0, serializedPath, [LedgerError.NoErrors])
      .then(processGetAddrResponse, processErrorResponse)
  }
